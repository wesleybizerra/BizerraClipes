...
export const api = {
  ...
  collectDailyReward: async (userId: string): Promise<{ message: string, user: User }> => {
    const response = await fetch(`${BACKEND_URL}/api/users/${userId}/daily-reward`, {
      method: 'POST'
    });
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || 'Erro ao coletar recompensa');
    return data;
  },
